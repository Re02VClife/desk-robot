...
最后更新：2026-06-23 22:20 | 分支：feature/robot-arm-vla
...
ESP32 端改动
文件	改动说明
protocol.h:76	新增 SendTextInput() 公开虚方法
protocol.cc:81-91	实现：用 cJSON 构建 {"type":"text","text":"..."} 消息
application.h:34	新增 MAIN_EVENT_TEXT_INPUT 事件位 (1<<13)
application.h:112	新增 SendTextInput() 公开方法（线程安全）
application.h:152	新增 HandleTextInputEvent() 处理器声明
application.cc:183	事件循环加入 TEXT_INPUT 监听
application.cc:1121-1177	HandleTextInputEvent 实现：按设备状态智能路由 + 15s 超时检测
新建 text_console.h	文字控制台头文件
新建 text_console.cc	FreeRTOS 任务：getchar() 逐字读取 + 回显 + 退格，回车发送
CMakeLists.txt:40	添加 text_console.cc
main.cc:24	TextConsole::Start() 启动控制台
OpenClaw 服务器端改动
文件	改动说明
textMessageType.py:9	新增 TEXT = "text" 消息类型枚举
新建 textInputMessageHandler.py	文字消息处理器：收到文字 → enqueue_asr_report + startToChat
textMessageHandlerRegistry.py:8,29	导入并注册 TextInputMessageHandler
数据流
串口终端 > 你好
...
飞书开放平台创建 Bot + 订阅 im.message.receive_v1 事件
OpenClaw 服务器新增 /feishu/callback HTTP 端点
飞书消息 → LLM → TTS → 双路回复（飞书文字 + ESP32 语音）
3. Phase 1 详细进度
3.1 ✅ 已完成
固件层
模块	文件	说明
UART1 驱动	xiaozhi_custom_board.cc:127	IO9(TX)→驱动板 RX, IO10(RX)←驱动板 TX, 115200bps
机械臂 MCP 工具	xiaozhi_custom_board.cc:150	robot.arm.move_joints、robot.arm.gripper、robot.arm.get_status 三个工具
WebSocket 地址覆盖	Kconfig.projbuild:915	Kconfig CONFIG_WEBSOCKET_URL_OVERRIDE，编译期覆盖 OTA 下发的 WS 地址
WebSocket 地址读取	websocket_protocol.cc:89	#if CONFIG_WEBSOCKET_URL_OVERRIDE 优先使用自建服务器地址
UART 引脚配置	config.h:75	ROBOT_ARM_UART_TXD_PIN=GPIO_NUM_9, ROBOT_ARM_UART_RXD_PIN=GPIO_NUM_10
分区表	partitions/v2/16m.csv	双 OTA (各 4MB) + 8MB SPIFFS assets
板卡选项	Kconfig.projbuild:133	BOARD_TYPE_XIAOZHI_CUSTOM，仅限 ESP32-S3
板级初始化	xiaozhi_custom_board.cc:197	构造函数中自动初始化 UART + 注册 MCP 工具
🆕 文字控制台	见上文 Phase 1.5	串口输入文字 → 直接与 AI 对话
文档
文件	说明
机械臂VLA集成方案.md	完整技术方案（674行），含架构、模型选型、微调流程
main/boards/xiaozhi-custom/README.md	板级说明（386行），含引脚映射、编译烧录、MCP 工具用法
🆕 SERVER_CHANGES.md	服务器端文字消息适配方案
项目整理
提交	说明
...
3.3 ⚠️ 已知问题
引脚不一致：方案文档 [机械臂VLA集成方案.md:127] 写 IO17(TX)/IO18(RX)，但实际固件 [config.h:78] 使用 IO9(TX)/IO10(RX)。原因可能是 IO17/IO18 实际布局在 CN1 扩展口不方便走线，改用更靠近主控的 IO9/IO10。需要同步更新方案文档。
get_status 返回 stub：robot.arm.get_status 当前只发送查询指令，返回固定 {"status":"pending"}，未真正解析驱动板回传的关节状态。需要驱动板支持查询协议后完善。
音量控制 API 已修复：原 xiaozhi_custom_board.cc 使用已废弃的 app.SetVolume()/app.GetVolume()，已改为 GetAudioCodec()->SetOutputVolume()，编译已通过。
bread-compact-wifi 固件 OLED 不工作：首次烧录用的是 bread-compact-wifi 板子类型，其 I2C SSD1306 引脚（IO41/IO42）与自定义板不符，导致 OLED 初始化失败。xiaozhi-custom 固件使用 SPI ST7789，需重新烧录验证。
文字输入后服务器无响应的超时处理：已加入 15 秒超时检测，超时后自动回 idle 并显示提示。需重启 OpenClaw 服务器加载新的 text handler 后才能正常工作。
4. Phase 2～4 规划（按优先级）
...
确认 OpenClaw 服务器已重启（加载 text handler）
打开串口监视器
看到 文字控制台已启动 提示
输入 你好 → 回车
等待服务器回复（TTS 语音 + 屏幕文字）
...
0. 🔴 烧录最新固件到 ESP32-S3（含文字控制台）
   ——当前步骤，需要烧录 feature/robot-arm-vla 分支最新固件

1. 🔴 重启 OpenClaw 服务器（加载 text handler）
   ——服务器端文字消息处理已写好，需重启生效

2. 🟡 测试文字对话
   ——串口输入 → OpenClaw → TTS 回复，端到端验证

3. 🔴 硬件接线 + UART 通信验证
   ——接线：ESP32 IO9/IO10 + GND ↔ SO101 驱动板串口
   ——验证：通过文字输入发送 MCP 指令，看机械臂是否响应

4. 🟡 统一文档引脚
   ——方案文档写 IO17/IO18，固件用 IO9/IO10，确认后修正方案文档

5. 🟠 部署 OpenClaw 后端（如未部署）
   ——Phase 2 启动前提，是所有语音/文字交互的基础

6. 🟢 评估 GPU 可用性
   ——Phase 3 的前置条件
   ——如果暂时没有 GPU，可以先用 Octo-Base（2GB CPU 也能跑）做流程验证

7. 🚀 可选：飞书 Bot 接入
   ——已有完整方案，需要飞书开放平台配置 + OpenClaw callback 端点
8. 关键资源
资源	地址	用途
本仓库	feature/robot-arm-vla 分支	ESP32 固件源码
OpenClaw 后端	C:\Users\24628\xiaozhi-esp32-server (本地)	LLM + TTS + MCP 调度
smolVLA	github.com/ZhangYizhe/smolVLA	VLA 视觉-语言-动作模型
LeRobot	github.com/huggingface/lerobot	SO101 仿真 + 数据采集 + 训练
LeRobot SO100 文档	lerobot.readthedocs.io	SO101 机械臂 SDK 参考
盒子桥教程	B站 BV1LN411K7Ps	SO101 DIY 入门
9. 里程碑时间线（预估）
2026年6月
  ├─ Week 4 (当前)
  │   ├─ ✅ 文字消息交互完成（ESP32 + 服务器）
