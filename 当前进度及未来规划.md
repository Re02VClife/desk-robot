...
> 最后更新：2026-07-06 22:00 | 分支：`feature/robot-arm-vla` / `feature/machine-dog`
...
| **Phase 1**   | 硬件接线 + ESP32 端 UART 驱动 + MCP 工具  | 🔄 进行中                       | ~90%   |
| **Phase 1.5** | 🆕 文字消息交互（串口 + 服务器适配）      | ✅ 完成                         | 100%   |
| **Phase 2**   | 部署 OpenClaw + 语音对话 + 语音控制机械臂 | 🔄 WebSocket直连+中文对话打通    | ~90%   |
| 🆕 **机器狗** | bread-compact-wifi 机器狗固件开发 + 4路舵机 + 触须传感器 | ✅ 编译通过，待硬件验证 | ~85% |
| 🆕 **ZZPET**  | ZzPetBot v1.3.4 固件逆向分析               | ✅ 提取完成，待 Ghidra 反编译 | ~40% |
| **Phase 3**   | 部署 smolVLA-0.5B + 视觉抓取推理          | 🟡 框架就绪（待 GPU）           | ~25%   |
| **Phase 4**   | 端到端语音抓取联调 + 异常处理             | 🟡 安全限位完成，其余待 Phase 3 | ~5%    |
| **仿真/微调** | LeRobot 遥操作数据采集 + LoRA 微调        | 🔲 未开始（可选）               | 0%     |

**整体进度：约 65%**（文字对话全链路打通，WebSocket直连，中文支持，VLA插件+安全限位完成）

> ✅ **已解决**：文字消息端到端畅通，"你好" → LLM "嗨～你好呀！今天过得怎么样？" 时延 ~3 秒。
> 🔴 **待做**：硬件接线 IO9/IO10 ↔ SO101 驱动板（控臂测试），GPU 采购（VLA 推理）
